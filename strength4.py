...
# new bug fixed Sep 16, 2013. Thanks Mingtao Li.
...
# rotate x
#  for i in range(len(b)):
#    print vecc[i][0],vecc[i][1],vecc[i][2]
  vecd[0][0] = vecc[0][0]
...
for i in range(1,int(step)):
  ...
  ftmp.close()
#  print >> ften, float(i)*float(strain), tmp2      
  if strength == "1":
      if i==1:
          print >> ften, '0.0 0.0'
  ...
  if strength == "2":
      if i==1:
          print >> ften, '0.0 0.0'
      print >> ften, ((1.0+float(strain))**float(i)-1.0)*2, tmp2
  cp_command = 'cp * %s' %i
  os.mkdir(str(i))
